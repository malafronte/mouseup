...
*/
//
// By 5IA of IISS Alessandro Greppi
// 2015/2016
// www.mouseup.it
...
#define MPU6050_I2C_ADDRESS 0x68    //Start addres of I2C

int delayTime = 50;

float gyro_angle_x, gyro_angle_y, gyro_angle_z, dt;
float tempgyrox = 0, tempgyroy = 0, tempgyroz = 0; 

float errordriftx1[100], errordrifty1[100], errordriftz1[100];
float  maximumDriftX, maximumDriftY, maximumDriftZ;

//Set pins of Arduino for connecting bluetooth
int TX = 11;
int RX = 10;
...
float errordriftofx[100], errordriftofy[100], errordriftofz[100];   //Array of medium error values

//---------------------------------------------------------------------------------
//---------------------------------------------------------------------------------
//      Start an initial calibration of drift     ---------------------------------
//---------------------------------------------------------------------------------
//---------------------------------------------------------------------------------
...
void setDrift() {
  ...
  for (int j = 0; j < 100; j++) {
    ...
    float RADIANS_TO_DEGREES = 180 / 3.14159; float accel_angle_y = atan(-1 * accel_x / sqrt(pow(accel_y, 2) + pow(accel_z, 2))) * RADIANS_TO_DEGREES;
    ...
    errordriftofx[j] = unfiltered_gyro_angle_x - tempgyrox;
    errordriftofy[j] = unfiltered_gyro_angle_y - tempgyroy;
    errordriftofz[j] = unfiltered_gyro_angle_z - tempgyroz;


    tempgyrox = unfiltered_gyro_angle_x;
    tempgyroy = unfiltered_gyro_angle_y;
    tempgyroz = unfiltered_gyro_angle_z;
  }
  maximumDriftX = medium(errordriftofx);
  maximumDriftY = medium(errordriftofy);
  maximumDriftZ = -0.1;
}

//---------------------------------------------------------------------------------
//---------------------------------------------------------------------------------
//  Return medium val of an array    ----------------------------------------------
//---------------------------------------------------------------------------------
//---------------------------------------------------------------------------------

float medium(float arr[]) {
  float max = arr[0];
...
void setup()
{
  ...
  Serial.begin(19200);
  Wire.begin();           // Initialize the 'Wire' for the I2C-bus.
...
//set to 0 all parameters used by the filter
float lastValOfGyrox = 0, lastValOfGyroy = 0, lastValOfGyroz = 0, driftOfGyrox = 0, driftOfGyroy = 0, driftOfGyroz = 0, lastValOfFiltredGyrox = 0, lastValOfFiltredGyroy = 0, lastValOfFiltredGyroz = 0, driftOfFiltredGyrox = 0, driftOfFiltredGyroy = 0, driftOfFiltredGyroz = 0, tempgyroxx = 0, tempgyroyy = 0, tempgyrozz = 0;
...
void loop()
{
  ...
  //  Calculate drift on every value of gyro catched                    -------------
  //  This is possible because delayTime is faster than head movements  -------------
  //---------------------------------------------------------------------------------
  //---------------------------------------------------------------------------------
  
  driftOfGyrox = gyro_x - tempgyrox;
  driftOfGyroy = gyro_y - tempgyroy;
  driftOfGyroz = gyro_z - tempgyroz;
  ...
  maximumDriftX = (driftOfGyrox);
  maximumDriftY = (driftOfGyroy);
  maximumDriftZ = driftOfGyroz;

  float DeltaGyrox = gyro_x - lastValOfGyrox;
  float DeltaGyroy = gyro_y - lastValOfGyroy;
  float DeltaGyroz = gyro_z - lastValOfGyroz;


  lastValOfGyrox = gyro_x;
  lastValOfGyroy = gyro_y;
  lastValOfGyroz = gyro_z;

  gyro_x -= (maximumDriftX);

  gyro_y += (maximumDriftY);
  gyro_z -= (maximumDriftZ);

  driftOfFiltredGyrox = unfiltered_gyro_angle_x - tempgyroxx;
  driftOfFiltredGyroy = unfiltered_gyro_angle_y - tempgyroyy;
  driftOfFiltredGyroz = unfiltered_gyro_angle_z - tempgyrozz;


  tempgyroxx = unfiltered_gyro_angle_x;
  tempgyroyy = unfiltered_gyro_angle_y;
  tempgyrozz = unfiltered_gyro_angle_z;


  maximumDriftX = (driftOfFiltredGyrox);
  maximumDriftY = (driftOfFiltredGyroy);
  maximumDriftZ = driftOfFiltredGyroz;

  float DeltaFilteredGyrox = unfiltered_gyro_angle_x - lastValOfFiltredGyrox;
  float DeltaFilteredGyroy = unfiltered_gyro_angle_y - lastValOfFiltredGyroy;
  float DeltaFilteredGyroz = unfiltered_gyro_angle_z - lastValOfFiltredGyroz;


  lastValOfFiltredGyrox = unfiltered_gyro_angle_x;
  lastValOfFiltredGyroy = unfiltered_gyro_angle_y;
  lastValOfFiltredGyroz = unfiltered_gyro_angle_z;
  ...
  Serial.print(F("errX:"));
  Serial.print(maximumDriftX);
  Serial.print(F("errY:"));
  Serial.print(maximumDriftY);
  Serial.print(F("errZ:"));
  Serial.println(maximumDriftZ);


  if ( bluetooth.available())   //if bluetooth is available...
  {
    //Send data
    //the 'u' character is delimitier for Android reading
    bluetooth.print(unfiltered_gyro_angle_x);
    bluetooth.print("u");
    bluetooth.print(unfiltered_gyro_angle_y);
    bluetooth.print("u");
    bluetooth.print(unfiltered_gyro_angle_z);
    bluetooth.print("u");
    bluetooth.println();
    //Serial.println("ciao");
  }
...
//---------------------------------------------------------------------------------
//---------------------------------------------------------------------------------
// MPU6050_read from an I2C device.  ----------------------------------------------
//---------------------------------------------------------------------------------
//---------------------------------------------------------------------------------
